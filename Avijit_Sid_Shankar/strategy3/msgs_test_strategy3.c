...
Filename 	:- msgs_test_strategy3.c 
Description 	:- This file spawns 2 clients and 2 servers. Message exchange happens between clients and servers. A client sends a message to the                server and gets a reply. 
*/


#include<stdio.h>
#include<stdlib.h>
#include "msgs_strategy3.h"
...
int main()
{
	...
	for(i = 0; i < 10; i++)
	{
		m[i].prod = CreateSem(10);
		m[i].cons= CreateSem(0);
		m[i].mutex1= CreateSem(1);
	}
	start_thread(client);
	start_thread(client);
        start_thread(server);
